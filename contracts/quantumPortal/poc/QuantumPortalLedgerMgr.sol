    ...
    IVersioned
{
    ...
    mapping(uint256 => QuantumPortalLib.RemoteTransaction[])
        public localBlockTransactions;
    mapping(uint256 => MinedBlock) public minedBlocks;
    mapping(uint256 => QuantumPortalLib.RemoteTransaction[])
        public minedBlockTransactions;
    ...
    function registerTransaction(
        ...
        if (_isLocalBlockReady(b)) {
            ...
            localBlocks[key] = LocalBlock({metadata: b});
        }
    ...
    function lastRemoteMinedBlock(uint64 chainId)
        external
        view
        returns (QuantumPortalLib.Block memory _block)
    {
    ...
    function localBlockByNonce(uint64 chainId, uint64 blockNonce)
        external
        view
        returns (LocalBlock memory, QuantumPortalLib.RemoteTransaction[] memory)
    {
        uint256 key = blockIdx(chainId, blockNonce);
    ...
    function doFinalize(
        ...
        QuantumPortalLib.Block memory lastFinB = lastFinalizedBlock[
            remoteChainId
        ];
        QuantumPortalLib.Block memory lastMinedB = lastMinedBlock[
            remoteChainId
        ];
        ...
        for (uint256 i = finalizeFrom; i <= blockNonce; i++) {
            ...
            // uint256 totalValue;
            finHash = keccak256(
                abi.encodePacked(finHash, minedBlocks[bkey].blockHash)
            );
     ...
     */
    function getBlockIdx(uint64 chainId, uint64 nonce)
        external
        pure
        returns (uint256)
    {
     ...
     */
    function executeBlock(uint256 key) internal {
        ...
        for (uint256 i = 0; i < transactions.length; i++) {
            ...
            uint256 baseGasUsed = qp.executeRemoteTransaction(
                i,
                b.blockMetadata,
                t,
                txGas
     ...
     */
    function blockIdx(uint64 chainId, uint64 nonce)
        private
        pure
        returns (uint256)
    {
    ...
    /**
     ...
     @return The price
     */
    function tokenPriceX128(address token)
        internal
        view
        virtual
        returns (uint256)
    ...
    /**
     ...
     @return The price
     */
    function localChainGasTokenPrice() internal view virtual returns (uint256) {
        return 1;
    }
     ...
     */
    function _isLocalBlockReady(QuantumPortalLib.Block memory b)
        private
        view
        returns (bool)
    {
