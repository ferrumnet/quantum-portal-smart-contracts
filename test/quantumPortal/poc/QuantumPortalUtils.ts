...
import { QuantumPortalLedgerMgrTest } from "../../../typechain-types";
import { QuantumPortalPocTest } from "../../../typechain-types";
...
export class QuantumPortalUtils {
    ...
    static async finalize(
        ...
        if (blockNonce > fin) {
            ...
            const authority = await authorityF.attach(authorityAddr) as unknown as  QuantumPortalAuthorityMgr;
...
export async function deployAll(): Promise<PortalContext> {
    ...
    const mgr1 = await mgrFac.deploy(26000, {gasLimit: 8000000}) as unknown as QuantumPortalLedgerMgrTest;
    console.log('MGR LAUNCHED', await mgr1.VERSION());
    const mgr2 = await mgrFac.deploy(2) as unknown as QuantumPortalLedgerMgrTest;
    ...
    const poc1 = await pocFac.deploy(26000) as unknown as QuantumPortalPocTest;
    const poc2 = await pocFac.deploy(2) as unknown as QuantumPortalPocTest;
    ...
    const autorityMgr1 = await deployUsingDeployer('QuantumPortalAuthorityMgr', ctx.signers.acc1.address,
        authInitData1, ctx.deployer, randomSalt(), ctx.signers.acc1) as unknown as QuantumPortalAuthorityMgr;
    const autorityMgr2 = await deployUsingDeployer('QuantumPortalAuthorityMgr', ctx.signers.acc2.address,
        authInitData2, ctx.deployer, randomSalt(), ctx.signers.acc2) as unknown as QuantumPortalAuthorityMgr;
    ...
    const feeConverter = await feeConverterF.deploy() as unknown as QuantumPortalFeeConverterDirect;
    await feeConverter.updateFeePerByte(Wei.from('0.001'));

    console.log('Deploying staking');
    const stake = await delpoyStake(ctx, autorityMgr1.target, ZeroAddress, tok1.target);
    ...
    const miningMgr1 = await deployMinerMgr(ctx, stake, poc1.target, mgr1.target, ctx.owner);
    const miningMgr2 = await deployMinerMgr(ctx, stake, poc2.target, mgr2.target, ctx.acc1); // To deploy a different contract.
    ...
    console.log(`Settting authority mgr (${autorityMgr1.target}/${autorityMgr2.target}) and miner mgr ${miningMgr1.address} / ${miningMgr2.address} on both QP managers, and fee converter`);
    ...
    const state1 = await stateF.deploy() as unknown as QuantumPortalState;
    await state1.setMgr(mgr1.target);
    await state1.setLedger(poc1.target);
    const state2 = await stateF.deploy() as unknown as QuantumPortalState;
    await state2.setMgr(mgr2.target);
    await state2.setLedger(poc2.target);

    await poc1.setManager(mgr1.target, state1.target);
    await poc1.updateFeeTarget();
    await poc1.setFeeToken(tok1.target);
    await miningMgr1.updateBaseToken(tok1.target);
    await poc2.setManager(mgr2.target, state2.target);
    ...
    await mgr1.updateState(state1.target);
    await mgr2.updateLedger(poc2.target);
    await mgr2.updateState(state2.target);
    console.log('Set.');
